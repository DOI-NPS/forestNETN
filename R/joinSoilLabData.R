...
joinSoilLabData <- function(park = 'all', from = 2007, to = as.numeric(format(Sys.Date(), "%Y")),
  ...
  tryCatch(soillab_vw <- get("SoilLab_NETN", envir = env) %>%
             ...
             filter(LabLayer %in% c("10 cm", "10cm", "10cm - NonVS", "A", "A - NonVS", "O", "O/A")) %>%
             filter(SampleYear > 2006
                    ),
  ...
  soil_merge2 <- soillab3 %>% left_join(., soil_samp_sum, by = intersect(names(.), names(soil_samp_sum))) %>%
                   mutate(Sample_Depth =
                            case_when(LabLayer == "O" & O_Hor_sum == 0 & A_Hor_sum == 0 ~ Total_sum,
                                      ...
                                      LabLayer %in% c("10 cm", "10cm", "O/A", "10cm - NonVS") & Horizon_QC == "O" & O_Hor_sum > 0 ~ O_Hor_sum,
                                      LabLayer %in% c("10 cm", "10cm", "O/A", "10cm - NonVS") & Horizon_QC == "O" & O_Hor_sum == 0
                                        & A_Hor_sum == Total_sum ~ A_Hor_sum,
                                      LabLayer %in% c("10 cm", "10cm", "O/A", "10cm - NonVS") & Horizon_QC == "O" &
                                      ...
                                      LabLayer %in% c("10 cm", "10cm", "O/A", "10cm - NonVS", "A - NonVS") &
